  ...
  useEffect(() => {
    const handleMessageReceived = (data) => {
      ...
      decryptMessage(newMessageReceived.content, signature, chatId).then(
        (res) => {
          const dataWithDecryptedContent = {
            ...newMessageReceived,
            content: res,
          };
          ...
          if (selectedChat && selectedChat._id === chatId) {
            setMessages((prevMessages) => {
              const newMessages = [...prevMessages, dataWithDecryptedContent];
              localStorage.setItem(
                `localMessages_${selectedChat._id}`,
            ...
            if (!notification.includes(dataWithDecryptedContent)) {
              setNotification((prev) => [...prev, dataWithDecryptedContent]);

              const localMessagesKey = `localMessages_${newMessageReceived.chat._id}`;
              const localMessages =
                JSON.parse(localStorage.getItem(localMessagesKey)) || [];
              const newMessages = [...localMessages, dataWithDecryptedContent];
              localStorage.setItem(
                localMessagesKey,
                JSON.stringify(newMessages)
              );
